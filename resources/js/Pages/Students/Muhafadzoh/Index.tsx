...
import { Plus, Edit, Trash2, BookOpen, Calendar, Search, FileDown, ChevronRight } from 'lucide-react';
...
export default function MuhafadzohIndex({ auth, studentsPaginated, students, filters }: any) {
    ...
    const presetGrades = ['A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D', 'E'];
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(route('muhafadzoh.index'), { search }, { preserveState: true });
    };
    ...
    const openModal = (muhafadzoh?: any, preselectedStudent?: any) => {
        ...
        if (muhafadzoh) {
            ...
            setTypeSearch('');
            setGradeSearch('');
            setData('date', format(new Date(), "yyyy-MM-dd"));
            
            if (preselectedStudent) {
                setData('student_id', preselectedStudent.id);
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        <form onSubmit={handleSearch} className="relative w-full md:w-96">
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2.5 rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white/50 backdrop-blur-sm"
                            />
                        ...
                        <div className="flex gap-3 w-full md:w-auto">
                            <a 
                                href={route('muhafadzoh.export-rekap')}
                                className="flex-1 md:flex-none inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 shadow-sm transition-all focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                            >
                                <FileDown className="w-5 h-5" /> Export Semua
                            </a>
                        </div>
                    </div>
